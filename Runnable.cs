    ...
    public class Runnable
    {
        ...
        public byte[] GetBytes()
        {
            ...
            using(MemoryStream ms = new MemoryStream())
            {
                ...
                ms.WriteByte(0);
                ms.Write(BitConverter.GetBytes(Header.Length).Reverse().ToArray());
                ms.Write(Encoding.UTF8.GetBytes(Header));
                ms.Write(BitConverter.GetBytes(Code.Length).Reverse().ToArray());
                ms.Write(Encoding.UTF8.GetBytes(Code));
                ms.Write(BitConverter.GetBytes(Footer.Length).Reverse().ToArray());
                ms.Write(Encoding.UTF8.GetBytes(Footer));
                ms.Write(BitConverter.GetBytes(Input.Length).Reverse().ToArray());
